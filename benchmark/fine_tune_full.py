...
def find_benchmark_dir(start: Path) -> Path:
    """
    Locate benchmark directory that contains benchmark_prompts.py and Dataset/.
    Supports:
      - cwd == benchmark/
    ...
    def ok(p: Path) -> bool:
        return (p / "Dataset").exists() and (p / "benchmark_prompts.py").exists()
    ...
    raise FileNotFoundError(f"Cannot locate benchmark/ with Dataset + benchmark_prompts.py from start={start}")
...
# ======================================================================================
DATA_ROOT = BENCHMARK_DIR / "Dataset"
